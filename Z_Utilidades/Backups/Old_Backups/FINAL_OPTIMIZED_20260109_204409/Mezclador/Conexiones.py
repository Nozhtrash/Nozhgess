...
def cols_mision(m: Dict[str, Any]) -> List[str]:
    ...
    cols += ["Familia", "Especialidad", "Caso", "Estado", "Apertura", "¿Cerrado?", "¿Apto?", "Mensual"]
...
def analizar_mision(sigges, m: Dict[str, Any], casos_data: List[Dict[str, Any]],
    ...
    # =========================================================================
    # 🧠 INTELIGENCIA DE HISTORIA (APTO + FOLIOS GLOBALES)
    # =========================================================================
    try:
        intel_data = buscar_inteligencia_historia(sigges, root, res["Estado"])
        res["¿Apto?"] = intel_data["apto"]
        ...
        log_warn(f"Fallo inteligencia historia: {e}")
        res["¿Apto?"] = "Error"
...
def procesar_paciente(sigges, row, idx, total, t_script_inicio: float) -> Tuple[List[Dict[str, Any]], bool]:
    ...
    try:
        ...
        if not resuelto:
            # 🔧 IMPROVED: Detailed skip reason + populated basic data
        ...
        resumen_paciente(
            ...
            res_paci, REVISAR_IPD, REVISAR_OA, MAX_REINTENTOS_POR_PACIENTE
        )
